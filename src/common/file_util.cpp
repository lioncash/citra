...
bool IsDirectory(const std::string &filename)
{
    ...
    if (result < 0) {
        LOG_WARNING(Common_Filesystem, "stat failed on {}: {}",
...
{
    LOG_INFO(Common_Filesystem, "File {}", filename);

    // Return true because we care about the file no
    // being there, not the actual delete.
    if (!Exists(filename))
    {
        LOG_WARNING(Common_Filesystem, "{} does not exist", filename);
        return true;
    }
    ...
    {
        LOG_ERROR(Common_Filesystem, "DeleteFile failed on {}: {}",
                   filename, GetLastErrorMsg());
        return false;
    }
#else
    if (unlink(filename.c_str()) == -1) {
        LOG_ERROR(Common_Filesystem, "Unlink failed on {}: {}",
                   filename, GetLastErrorMsg());
        return false;
    }
#endif
...
bool CreateDir(const std::string &path)
{
    ...
    {
        LOG_WARNING(Common_Filesystem, "CreateDirectory failed on {}: already exists", path);
        return true;
    }
    ...
    if (err == EEXIST)
    {
        LOG_WARNING(Common_Filesystem, "mkdir failed on {}: already exists", path);
        return true;
    }
...
bool CreateFullPath(const std::string &fullPath)
{
    ...
    LOG_TRACE(Common_Filesystem, "Path {}", fullPath.c_str());

    if (FileUtil::Exists(fullPath))
    {
        LOG_WARNING(Common_Filesystem, "Path exists {}", fullPath);
        return true;
    }
...
{
    LOG_INFO(Common_Filesystem, "Directory {}", filename);
...
#define BSIZE 1024
    ...
    if (!input)
    {
        LOG_ERROR(Common_Filesystem, "Opening input failed {} --> {}: {}",
                  srcFilename, destFilename, GetLastErrorMsg());
        return false;
    }
    ...
    {
        fclose(input);
        LOG_ERROR(Common_Filesystem, "Opening output failed {} --> {}: {}",
                  srcFilename, destFilename, GetLastErrorMsg());
        return false;
    }
...
u64 GetSize(const std::string &filename)
{
    ...
    LOG_ERROR(Common_Filesystem, "Stat failed {}: {}",
              filename, GetLastErrorMsg());
    return 0;
}
...
u64 GetSize(const int fd)
{
    struct stat64 buf;
    if (fstat64(fd, &buf) != 0) {
        LOG_ERROR(Common_Filesystem, "GetSize: stat failed {}: {}",
                  fd, GetLastErrorMsg());
        return 0;
    }
    return buf.st_size;
...
u64 GetSize(FILE *f)
{
    ...
    if (fseeko(f, 0, SEEK_END) != 0) {
        LOG_ERROR(Common_Filesystem, "GetSize: seek failed {}: {}",
                  static_cast<void*>(f), GetLastErrorMsg());
        return 0;
    }
    u64 size = ftello(f);
...
#endif
        LOG_ERROR(Common_Filesystem, "GetCurrentDirectory failed: {}",
                  GetLastErrorMsg());
        return nullptr;
    }
#ifdef _WIN32
